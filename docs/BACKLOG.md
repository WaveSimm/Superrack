...
- **미확인**: 실기 청취(경계 클릭 여부), 특히 스템 SR ≠ 장치 SR 인 리샘플 재생.

### L. 타임라인 UI 재정비  [P1] — **설계 필요(2026-08-14 예정)**
현재 진행바 한 줄에 성격이 다른 정보가 겹쳐 읽기 어렵다. **기존 DAW 의 타임라인 구성을
참조해 다시 설계**한다.

지금 겹쳐 있는 것:
- `positionSlider` — 재생 위치/시크
- `PunchStripOverlay` — 녹음 이력. **마지막 항목 = 주황, 이전 = 회색**(색 = 상태가 아니라 순서)
- `LoopRangeStrip` — 반복 구간(초록/회색)

풀어야 할 문제:
- **회색 이력이 오해를 부른다.** 펀치인은 그 지점부터 **끝까지 덮어쓰므로**(`TakeManager.cpp`
  commitRecording 주석) 이전 이력 구간이 지금 들리는 오디오와 다르다. 30초 테이크의 10초에서
  5초를 펀치인하면 결과는 15초인데, 회색 막대는 0~30초로 남아 현재 오디오처럼 보인다.
- 반복 구간까지 같은 자리에 얹혀 세 정보가 경쟁한다.
- 막대에 시각·구간 정보가 없다(툴팁 없음).

검토할 방향: 회색 이력 제거(주황=현재 유효 구간만) / 레인 분리(룰러·반복·클립) /
툴팁 또는 범례 / DAW 관행(룰러 상단 루프 브레이스, 클립 레인, 플레이헤드) 차용.
